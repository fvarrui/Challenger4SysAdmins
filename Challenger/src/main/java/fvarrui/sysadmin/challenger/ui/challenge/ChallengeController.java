...
public class ChallengeController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		startButton.disableProperty().bind(challenge.isNull().or(running));			// deshabilita el botón de inicio si no hay reto o está ejecutándose
		pauseButton.disableProperty().bind(challenge.isNull().or(running.not()));	// deshabilita el botón de pausa si no hay reto o está parado
		restartButton.disableProperty().bind(challenge.isNull());					// deshabilita el botón de reinicio si no hay reto
	...
	/** 
	 * Evento que se dispara el cambiar la descripción del reto
	 * @param ov
	 * @param nv
	 */
	 ...
	 * @param nv Nueva descripción
	 */
	private void onGoalDescriptionChanged(String nv) {
		if (nv != null) {
			Platform.runLater(() -> goalView.getEngine().loadContent(MarkdownUtils.render(nv)));
		} else {
			Platform.runLater(() -> goalView.getEngine().loadContent(MarkdownUtils.render(":warning: No hay ningún objetivo seleccionado.")));
		}
	}
	...
	private void onChallengeChanged(Challenge ov, Challenge nv) {
		// comprueba si había un reto cargado
		if (ov != null) {
	...
	/**
	 * Evento que se dispara al pulsar el botón de iniciar (play)
	 * @param e
	 */
	@FXML
	...
	/**
	 * Evento que se dispara al pulsar el botón de pausa (pause)
	 * @param e
	 */
	@FXML
	...
	/**
	 * Evento que se dispara al pulsar el botón de reiniciar (restart)
	 * @param e
	 */
	@FXML
	...
	/**
	 * Evento que se dispara al pulsar el botón de cerrar (close)
	 * @param e
	 */
	@FXML
	...
	/**
	 * Evento que se dispara al pulsar el botón de configuración (settings)
	 * @param e
	 */
	@FXML
	...
	// TODO poner un botón para cerrar el reto actual y volver a la ventana de carga
	
	// get root view

	/**
	 * Obtener el nodo raíz de la vista asociada al controlador
	 * @return
	 */
